...
NABTO_CONFIG_FILE=/tmp/etc/nabtonat_config
...
SetNatP2p()
{
	if [ -f "/sys/class/net/wlan0/address" ]; then
		HWaddr=`cat "/sys/class/net/wlan0/address" | awk -F: '{print $4 $5 $6}' | awk '{print $1}'`
	else
		return 1
	fi
	if [ ! -f $NABTO_CONFIG_FILE ]; then
		NABTO_UUID=lx520.$HWaddr.p2p.rakwireless.com
	...
	if [ -f $NABTO_CONFIG_FILE ]; then
		if [ "$NABTO_KEY" == "" ]; then
		./unabto_tunnel -d $NABTO_UUID -s &
		else
		./unabto_tunnel -d $NABTO_UUID -k $NABTO_KEY -s &
		fi
	else
		echo "Can't find $NABTO_CONFIG_FILE"
	fi
